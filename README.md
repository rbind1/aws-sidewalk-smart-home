...
6. [Security Keys and Tokens](#8-security-keys-and-tokens)
...
A low-power smart room node placed on a nightstand, shelf, or windowsill that:

- Reads temperature and humidity on press using a Bosch BME280 sensor
...
| Uplink interval (nominal) | on-press |
| Payload size | 6 bytes binary |
| Cloud infrastructure cost | < $0.05 / day (depends on workload) |
| Sidewalk availability | US only! |
| Gateway requirement | Echo 3rd gen+ or Ring Bridge PRO |
...
Note: Do not commit AWS credentials, manufacturing blobs, private keys, access keys, or certificates.
...
```powershell
cd C:~~\sidewalk-addon\sidewalk\samples\sid_end_device
```
