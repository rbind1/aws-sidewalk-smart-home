...
#define SENSOR_PAYLOAD_MAX_LEN 160

static uint32_t persistent_link_mask;

static const struct device *const bme280_dev = DEVICE_DT_GET(DT_NODELABEL(bme280));

static int sensor_value_to_fixed6(char *buf, size_t buf_len, const struct sensor_value *val)
{
	int64_t micro_value = ((int64_t)val->val1 * 1000000LL) + val->val2;
	uint64_t abs_value;
	const char *sign = "";

	if (micro_value < 0) {
		sign = "-";
		abs_value = (uint64_t)(-micro_value);
	} else {
		abs_value = (uint64_t)micro_value;
	}

	return snprintk(buf, buf_len, "%s%llu.%06llu",
			sign,
			(unsigned long long)(abs_value / 1000000ULL),
			(unsigned long long)(abs_value % 1000000ULL));
}

static bool bme280_is_ready(void)
{
	if (!device_is_ready(bme280_dev)) {
		LOG_ERR("BME280 device is not ready. Check wiring, overlay, and I2C address.");
		return false;
	}
...
static int bme280_read_values(struct sensor_value *temp,
			      ...
			      struct sensor_value *humidity)
{
	...
	/*
	 * The BME280 driver can occasionally return -EAGAIN,
	 * which means the measurement was not ready yet.
	 * Wait briefly and try once more.
	 */
	if (err == -EAGAIN) {
		LOG_WRN("BME280 sample not ready, retrying...");
		k_sleep(K_MSEC(100));
		err = sensor_sample_fetch(bme280_dev);
	}

	if (err) {
		LOG_ERR("BME280 sample fetch failed after retry: %d", err);
		return err;
	}

	err = sensor_channel_get(bme280_dev, SENSOR_CHAN_AMBIENT_TEMP, temp);
	if (err) {
		LOG_ERR("BME280 temperature read failed: %d", err);
		return err;
	}

	err = sensor_channel_get(bme280_dev, SENSOR_CHAN_PRESS, press);
	if (err) {
		LOG_ERR("BME280 pressure read failed: %d", err);
		return err;
	}

	err = sensor_channel_get(bme280_dev, SENSOR_CHAN_HUMIDITY, humidity);
	if (err) {
		LOG_ERR("BME280 humidity read failed: %d", err);
		return err;
	}
...
static int bme280_build_payload(char *payload, size_t payload_len)
{
	...
	err = bme280_read_values(&temp, &press, &humidity);
	if (err) {
		return snprintk(payload, payload_len,
				"{\"sensor\":\"bme280\",\"status\":\"error\",\"err\":%d}",
				err);
	}

	(void)sensor_value_to_fixed6(temp_str, sizeof(temp_str), &temp);
	(void)sensor_value_to_fixed6(press_str, sizeof(press_str), &press);
	(void)sensor_value_to_fixed6(humidity_str, sizeof(humidity_str), &humidity);

	LOG_INF("BME280 temp: %s C", temp_str);
	LOG_INF("BME280 pressure: %s kPa", press_str);
	LOG_INF("BME280 humidity: %s %%", humidity_str);

	return snprintk(payload, payload_len,
			"{\"sensor\":\"bme280\",\"temp_c\":%s,\"pressure_kpa\":%s,\"humidity_pct\":%s}",
			temp_str, press_str, humidity_str);
}

static void bme280_startup_test(void)
{
	char payload[SENSOR_PAYLOAD_MAX_LEN];
	int len;

	LOG_INF("Checking BME280 sensor");

	if (!bme280_is_ready()) {
		return;
	}

	len = bme280_build_payload(payload, sizeof(payload));
	if (len < 0 || len >= sizeof(payload)) {
		LOG_ERR("BME280 startup payload formatting failed");
		return;
	}

	LOG_INF("BME280 startup payload: %s", payload);
}
...
static void free_sid_sensor_event_ctx(void *ctx)
{
	sidewalk_msg_t *sensor_msg = (sidewalk_msg_t *)ctx;
	if (sensor_msg == NULL) {
		return;
	}

	if (sensor_msg->msg.data) {
		sid_hal_free(sensor_msg->msg.data);
	}

	sid_hal_free(sensor_msg);
}
...
static void app_btn_send_msg(uint32_t unused)
{
	...
	char payload[SENSOR_PAYLOAD_MAX_LEN];
	int payload_len = bme280_build_payload(payload, sizeof(payload));

	if (payload_len < 0) {
		LOG_ERR("Failed to format BME280 payload");
		return;
	}

	if (payload_len >= sizeof(payload)) {
		LOG_ERR("BME280 payload too large: %d", payload_len);
		return;
	}

	LOG_INF("Send BME280 sensor message: %s", payload);

	sidewalk_msg_t *sensor_msg = sid_hal_malloc(sizeof(sidewalk_msg_t));
	if (!sensor_msg) {
		LOG_ERR("Failed to alloc memory for message context");
		return;
	}

	memset(sensor_msg, 0x0, sizeof(*sensor_msg));

	sensor_msg->msg.size = strlen(payload);
	sensor_msg->msg.data = sid_hal_malloc(sensor_msg->msg.size);
	if (!sensor_msg->msg.data) {
		sid_hal_free(sensor_msg);
	...
	memcpy(sensor_msg->msg.data, payload, sensor_msg->msg.size);

	sensor_msg->desc.type = SID_MSG_TYPE_NOTIFY;
	sensor_msg->desc.link_type = SID_LINK_TYPE_ANY;
	sensor_msg->desc.link_mode = SID_LINK_MODE_CLOUD;

	int err = sidewalk_event_send(sidewalk_event_send_msg, sensor_msg, free_sid_sensor_event_ctx);
	if (err) {
		free_sid_sensor_event_ctx(sensor_msg);
		LOG_ERR("Send event err %d", err);
	} else {
#if defined(CONFIG_STATE_NOTIFIER)
		application_state_sending(&global_state_notifier, true);
